...
\textcolor{blue}{Alex's final percentile could be 1 (better than or equal to only one student: himself). There are 99 other students, and Alex scores in the $91^{st}$ percentile, so he scores better than or equal to 91 students on every exam, but there are 9 students who score better than him. Suppose that on the first quiz 90 students get a 10, he gets an 11, and 9 students get 100. Now, on the next quiz, suppose 9 of the students who previously got 10s get 100s, the people who got 100s get 10s, and Alex gets an 11 again. Repeat this 9 more times with a new group of 9 people each time. Then, Alex's average is an 11, but the average for every other student is $\frac{10\cdot10+100}{11}\approx 18.2.$}
...
```{r, warning=F, cache=T, fig.width=8, fig.height=5, echo=F}
library(ggplot2)
set.seed(33)
n = 20
sigma = 1
...
\textcolor{blue}{Even though there is one group that appears to have higher values than the rest, there are enough groups that this will probably be insignificant. (Note that the code to generate these data intentionally placed this group higher than the rest and that the test indeed came out insignificant.)}
...
\textcolor{blue}{Assuming that the group on the right is almost always significantly different from the rest of the groups and that the rest of the groups have an $\alpha=0.05$ probability of a false positive difference, the proportion of rejections will be about $\frac{59}{{60\choose 2}}+0.05\left(1-\frac{59}{{60\choose 2}}\right)=0.082$. Indeed, the pairwise test show that 0.072 (closest to 0.082) are significant}

```{r, cache=T}
mean(pairwise.t.test(data$Value, data$Group, 
                     p.adjust.method = "none")$p.value < 0.05, na.rm=T)
```
...
\textcolor{blue}{All of these follow the $t_9$ distribution. The first is our usual $t$-statistic. The second is the usual $t$-statistic in standard error notation. The third uses the fact that $\frac{\textrm{UB}-\bar{X}}{t^*_9}=\textrm{SE}(\bar{X})$. The fourth uses the fact that the p-values are uniform under the null since the $t_9$ CDF is continuous.}
...
(f) For each variable, choose the transformation most likely to make it normal from the following list: log, exponential, square root, logit, reciprocal.
  ...
  ii. Population in US cities: \textcolor{blue}{Log, likely to be very right skewed}
...
(g) For each of the following scenarios, choose the 2-sample comparison from the following list most likely to fix any issues with the original data. Use each test one time: unpooled $t$-test, paired $t$-test, rank-sum test, and permutation test for means.
  ...
  iii. The values in each group are measurements of the same item at two times (one time in each group). You want to compare the means of the groups. \textcolor{blue}{Paired $t$-test: You have two measurements for each item, so there is a natural pairing.}
  iv. Each group has 80 observations, and the distribution of each group's values is slightly right skewed. You want to compare the means of the groups. \textcolor{blue}{Unpooled $t$-test: There are many observations, and if the data are not very skewed, the distributions of the sample mean will be approximately normal.}
...
\textcolor{blue}{This is a classic case of regression to the mean. When a pilot performs very poorly on one flight, the pilot will often perform better on the next just by chance. When a pilot performs unusually well on a flight, the pilot is unlikely to perform even better on the next. Kahneman was noting that in the long run, rewarding good performance tends to produce better results.}
...
```
\textcolor{blue}{The correlation of the residuals and predictors is 0 in all cases since residuals are always uncorrelated with the predictors in an OLS model with an intercept. (If they were correlated, this would've been incorporated into the $\hat{\beta}$ prediction.) The first case is a violation of linearity; the second is a violation of homoskedasticity; the third is a violation of both homoskedasticity and linearity.}
...
\textcolor{blue}{A is not equivalent to the rest (and is in fact 0). For the rest, note that $$\begin{aligned}\sum_{i=1}^n(X_i-\bar{X})^2&=\sum_{i=1}^n(X_i^2-2X_i\bar{X}+\bar{X}^2)\\
&=\left(\sum_{i=1}^nX_i^2\right)-2\bar{X}\left(\sum_{i=1}^nX_i\right)+\left(\sum_{i=1}^n\bar{X}^2\right)\\
&=\left(\sum_{i=1}^nX_i^2\right)-2n\bar{X}^2+n\bar{X}^2\\
&=\left(\sum_{i=1}^nX_i^2\right)-n\bar{X}^2\\
&=\left(\sum_{i=1}^nX_i^2\right)-\bar{X}\sum_{i=1}^nX_i\\
&=\sum_{i=1}^n\left(X_i-\bar{X}\right)X_i
\end{aligned}$$}
...
\textcolor{blue}{The contrast test uses the statistic $$T=\frac{\vv{C}^T\hat{\vv{\beta}}}{\hat{\sigma}\sqrt{C^T(X^TX)^{-1}C}}=\frac{\hat{\beta_1}}{\hat{\sigma}\sqrt{1/\textrm{SSX}}}$$ ($C^T(X^TX)^{-1}C=\frac{1}{\textrm{SSX}}$ can be seen from Pset 4.)}
...
\textcolor{blue}{Ben should use a $z$-based confidence interval: $$\left(\hat{p}-z^*\sqrt{\frac{\hat{p}(1-\hat{p})}{n}}, \hat{p}+z^*\sqrt{\frac{\hat{p}(1-\hat{p})}{n}}\right)=(0.6-1.96\cdot0.089, 0.6+1.96\cdot0.089)=(0.425, 0.775)$$ We are assuming his sleep times are independent each night, which might be questionable. He stayed up past midnight 18 times and went to sleep before midnight 12 times, so the regularity condition (>10) is satisfied, and we can use a Normal approximation to the binomial. }
...
which is indeed slightly wider than before. The trick here is that because the data is binomial, we know the exact sampling distribution when bootstrapping. Usually this isn't the case.}
...
\textcolor{blue}{Making more than 12 points averaged across the games is equivalent to making 25 or more points total. The sum of independent Poissons is Poisson with the sum of their parameters, so the number of points Alice makes in both games combined has the distribution $\textrm{Pois}(36)$. The new probability of a Type 1 error is then $$\sum_{k=0}^{24}\frac{e^{-36}36^k}{k!}\approx0.022$$  Likewise, the probability of a Type 2 error is $$1-\sum_{k=0}^{24}\frac{e^{-18}18^k}{k!}\approx0.068$$}
...
\textcolor{blue}{$$\hat{\beta}_{1, y\sim x}=\frac{r_{XY}S_Y}{S_X}$$ which is not symmetric in $X$ and $Y$, so the slopes will only be the same in the two models if $S_X=S_Y$. Additionally, the $t$-statistics are equal, so $\frac{\hat{\beta}_{1, y\sim x}}{\textrm{SE}(\hat{\beta}_{1, y\sim x})}=\frac{\hat{\beta}_{1, x\sim y}}{\textrm{SE}(\hat{\beta}_{1, x\sim y})}$, so the standard errors need not be the same unless the slope estimates are the same.}
